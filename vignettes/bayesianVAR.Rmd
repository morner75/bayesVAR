...
$\texttt{bayesVAR}$ 패키지는 베이지안 벡터자기회귀모형에 대한 추론을 위한 패키지이다.
...
이번 장에서는 벡터자귀회귀(Vector Autoregressive: VAR) 모형의 설정 및 추정방법에 대하여 다룬다.  $k$개의 내생변수와 $p$개의 시차, $m$​개의 외생변수를 갖는 벡터자귀회귀 모형은 다음과 같이 나타낼 수 있다.
$$
\begin{aligned}
\begin{pmatrix}
y_{1,t}\\
...
>$$
>\hat{\bm A}_1=\begin{pmatrix}0.911 & -0.102\\-0.004&1.168\end{pmatrix},\quad \hat{\bm A}_2=\begin{pmatrix}-0.254 & 2.743\\0.0054&-0.323\end{pmatrix},\quad \hat{\bm C}=\begin{pmatrix}-1.540&162.480 \\0.2939& 0.213\end{pmatrix},\quad \hat{\bs\Sigma}=\begin{pmatrix}1.785 & -25.025\\-25.025& 2423.148\end{pmatrix}
>$$
...
$$
\begin{aligned}
L(\bm y\mid \bs\beta,\bs\Sigma)&\propto |\bs\Sigma|^{-(kp+m)/2}\exp\left[-\frac{1}{2}(\bs\beta -\hat{\bs\beta})^T(\bs\Sigma\otimes(\bm X ^T \bm X)^{-1})^{-1}(\bs\beta -\hat{\bs\beta})\right]\times|\bs\Sigma|^{-[(T-(kp+m)-k-1)+k+1]/2}\exp\left[-\frac{1}{2}\text{tr}\left\{\bs\Sigma^{-1}(\bm Y-\bm X \hat{\bm B})^T(\bm Y-\bm X \hat{\bm B})\right\}\right]\\
...
> (알고리즘 1) 회귀계수 $\bs\beta$ 와 공분산 $\bs\Sigma$ 의  Gibbs 샘플링 
>
