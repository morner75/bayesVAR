...
`bayesVAR` 패키지는 거시변수 데이터 $\texttt{macrodata}$를 제공하는데, 실질GDP성장율(RGDP_Q), 원달러환율변화율(USDKRW_G) 등 2000년 3분기부터 2021년 2분기까지 주요 분기별 자료를  $\texttt{xts}$ 시계열 형태로 제공한다.
...
이제 실질GDP(RGDP_G), 원달러환율(USDKRW_G)가 내생변수이고, 콜금리(INT_CALL)가 외생변수인 시차 2인 VAR모형을 `bayesVAR` 패키지를 이용하여 추론해 보자. 
